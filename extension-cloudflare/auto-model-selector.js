...
(function (root) {
  ...
  const MODEL_NAME_REGEX = /(\d+(?:\.\d+)*)\s*(Pro|Flash(?:-Lite)?)/i;
  ...
  function openModelPicker() {
    ...
    if (!trigger) {
      console.warn("[Bridge] Model picker button not found in DOM");
      return null;
    }
  ...
  function selectModel(preferredFamily = "pro", minVersion = 0) {
    try {
      ...
      if (options.length === 0) {
        console.warn("[Bridge] No model options found in dropdown");
